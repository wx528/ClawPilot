...
- **执行器健康检查** — 实时检测各执行器可用性，批量健康检查，异常自动上报

- **可配置重试策略** — 支持指数退避、固定间隔、线性递增三种重试策略，可配置最大重试次数与延迟上限

- **任务日志持久化** — 执行日志自动写入 SQLite，支持按任务查询、近期查询、过期清理

- **零依赖部署** — 单个 exe，开箱即用
- **内嵌 SQLite** — 任务队列 + 编排数据库，本地存储
- **进程管理** — 直接管理各执行器进程；超时可配置，失败任务自动重试
- **任务编排** — Persona / Prompt / DailyPlan 完整 CRUD
...
└── ClawPilot.Core (核心业务库)
    ...
    │
    └── Services/ — 核心服务
        ├── IExecutor.cs — 执行器接口 + 健康检查
        ├── ExecutorRegistry.cs — 执行器注册中心
...
3. **注册 DI** — `App.xaml.cs` 注册、`ExecutorRegistry` 注册

基类自动提供：PATH 查找（`where`/`which`）、Windows UTF-8 编码修复（`chcp 65001`）、超时控制、输出收集、参数转义、健康检查。
